  ...
  $.FE.PLUGINS.url = function (editor) {
    ...
    function _init () {
      editor.events.on('paste.afterCleanup', function (html) {
        ...
        var walker = editor.doc.createTreeWalker(div, NodeFilter.SHOW_TEXT, editor.node.filter(function (node) {

            return (new RegExp($.FE.URLRegEx,'gi')).test(node.textContent);
          }), false);
        ...
        for (var i = 0; i < texts.length; i++) {
          var nd = texts[i];
          $(nd).after(_convertToLink(nd.textContent)).remove();
        }
